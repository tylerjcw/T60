...
{
    for exp in $argv; do
        print "$exp = ${grn}$(bc <<<$exp)${rst}"
    done
}

function netup() #== Returns wether network is up or down
{
...
{
    if mountpoint -q $HOME/srv/storage; then
        echo false
    else
        echo true
    fi
...
{
    if mountpoint -q $HOME/srv/minecraft; then
        echo false
    else
        echo true
    fi
...
function makestatus() #== Compiles dwmst and copies the resulting binary to ~/bin
{
    (pushd $HOME/code/dwmst; make uninstall clean; make; make install clean; popd) &> /dev/null
    pkill dwmst
    $HOME/bin/dwmst & disown
